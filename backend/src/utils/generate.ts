...
export function replace_prompt_variables(text:string, variable_object:object) {
    var template = handlebars.compile(text);
    var new_text = template(variable_object);
    return new_text
}

export function process_prompt_variables(prompt: any, body: any, proxy: boolean) {
    ...
    if (prompt_data['context']) {
        prompt_data['context'] = replace_prompt_variables(prompt_data['context'], prompt_variables);
    }
    if (prompt_data['prompt']) {
        prompt_data['prompt'] = replace_prompt_variables(prompt_data['prompt'], prompt_variables);
    }
    if (prompt_data['messages']) {
        ...
        for (var i = 0; i < prompt_data['messages'].length; i++) {
            prompt_data['messages'][i]['content'] = replace_prompt_variables(prompt_data['messages'][i]['content'], prompt_variables);
        }
    }
...
export async function prompt_model_validation(body:any, organization:any) {
    ...
    try {
        ...
        if(!prompt){
            return [undefined, undefined, undefined, { error:true, message: 'Prompt not found.', status: 404 }, organization, cache]
        }
        ...
        if(!model){
            return [undefined, undefined, undefined, { error:true, message: 'Model not found.', status: 404 }, organization, cache]
        }
        ...
        return [undefined, undefined, undefined, { error:true, message: error, status: 500 }, organization, cache]
...
export async function generate_cached_response(body:any, cache:any, organization:any) {
