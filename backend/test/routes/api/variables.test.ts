...
describe('Variable API', function() {
  ...
  before(async () => {
    ...
    await variableDbInstance.createVariables(initialVariableData.data, organization.id);
  });
  ...
  it('should respond with 200 and the variable JSON on GET /variables', async function() {
    const res = await request(app).get('/api/variables').set('Authorization', 'Bearer ' + token);
  ...
  it('should update the variable and respond with 200 on PUT /variables', async function() {
    ...
    const res = await request(app).put('/api/variables').send(updatedVariableData).set('Authorization', 'Bearer ' + token);
    expect(res.status).to.equal(200);
