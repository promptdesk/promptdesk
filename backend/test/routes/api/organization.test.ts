...
describe('Organization API', function() {
  ...
  it('should respond with 200 and the organization JSON object on GET /organization', async function() {
    const res = await request(app).get('/api/organization').set('Authorization', 'Bearer ' + token);
    expect(res.status).to.equal(200);
    expect(res.body).to.be.an('object');
  });

  it('should delete the organization and respond with 200 on DELETE /organization', async function() {
    const res = await request(app).delete(`/api/organization/${organization.id}`).set('Authorization', 'Bearer ' + token);
    expect(res.status).to.equal(200);
    expect(res.body).to.have.property('message', 'Organization deleted successfully');
  });
