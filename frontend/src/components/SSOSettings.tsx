...
const SSOSettings: React.FC = () => {
    ...
    const { isSSO, fetchIsSSO, saveSSO, organization } = organizationStore.getState();
    const [clientID, setClientID] = React.useState('');
    const [redirectEndpoint, setRedirectEndpoint] = React.useState((process.env.PROMPT_SERVER_URL || window.location.origin) + '/auth/sso');
    ...
    return (
        <div>
            ...
            {isSSO !== true && <Warning text="To fully enable this optional feature, you must set SSO_CLIENT_SECRET as an environemnt variable." />}
            {isSSO === true && (
            <>
                <Success text="SSO enabled. SSO_CLIENT_SECRET is set." />
                <br />
                <InputField label="Provider" value={provider} onInputChange={(value) => setProvider(value)}/><br />
                <InputField label="Redirect Endpoint" value={redirectEndpoint} onInputChange={(value) => setRedirectEndpoint(value)}/><br />
