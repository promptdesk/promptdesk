...
const SampleRow: React.FC<any> = ({ index, sample }) => {
  const [localResult, setLocalResult] = React.useState<any>(
    sample.result || "",
  );
  const [localStatus, setLocalStatus] = React.useState<any>(
    sample.status || "new",
  );
  const [localPromptInfo, setLocalPromptData] = React.useState<any>(
    sample.prompt || {},
  );
  const [isShowingDeleteModal, setIsShowingDeleteModal] = React.useState(false);
  const [isShowingConfirmGenerateModal, setIsShowingConfirmGenerateModal] =
  ...
  const saveChangedResultValue = useMemo(
    () =>
      _.debounce((newResultValue) => {
  ...
  const saveChangedPromptData = useCallback(
    (newPromptData: any) => {
      sampleStore.getState().patchSample(sample_id, {
  ...
  async function regenerateResultValue() {
    ...
    Object.keys(prompt.prompt_variables).map((variable_name: string) => {
      if (sample.variables[variable_name] !== undefined) {
        prompt.prompt_variables[variable_name].value =
          sample.variables[variable_name];
      }
    });
  ...
  const promptText = (localPromptInfo.prompt || localPromptInfo.content || "")
    .toString()
    .trim();
