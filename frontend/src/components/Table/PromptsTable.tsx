...
const PromptsTable: React.FC<PromptsTableProps> = ({ promptList }) => {
  ...
  useEffect(() => {
    ...
    promptLista = Object.keys(promptLista).sort().reverse().reduce(
      (obj: any, key) => {
          ...
          <>
            {((project_key !== "undefined" && project == "undefined") || ((project == "undefined") && project_key !== "undefined")) &&
              <tr onClick={() => {push(`/prompts/${project_key}`)}} className="cursor-pointer">
                <td className="whitespace-nowrap w-full py-4 px-4 text-sm text-gray-500 bg-gray-100 hover:bg-gray-200" colSpan={5}>
            ...
            {(prompts as any)[project_key].map((prompt:any) => (
              project_key !== project || prompt.new === true ? null : (
                <tr key={prompt.id} onClick={() => {push(`/workspace/${prompt.id}`)}} className="cursor-pointer hover:bg-gray-50">
                  ...
                  <td className="whitespace-nowrap py-4 px-4 text-sm text-gray-500">{prompt.provider}</td>
                </tr>
              )
            ))}
          </>
