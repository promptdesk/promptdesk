...
const PromptsTableTempporaryRefactor: React.FC<PromptsTableProps> = ({ promptList }) => {
    ...
    const folders: IFolder[] = _.uniqBy(promptList.
        filter(prompt => {
            ...
            return projSplit.length === level + 1 && projSplit[level - 1] === paths[level - 1];
        }).
        map(prompt =>
        ({
