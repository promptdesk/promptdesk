...
export async function createNewPrompt() {
    ...
    if (!isValidName(promptObject.name) || (promptObject.project && !isValidName(promptObject.project))) return;
    ...
    tabStore.getState().updateNameById(promptObject.id, data.id, promptObject.name);
    return data.id;
}
...
export async function updateExistingPrompt() {
    ...
    await fetchAndSetPrompts();
    tabStore.getState().updateNameById(promptObject.id, promptObject.id, promptObject.name);
}
...
export async function duplicateExistingPrompt(name:any, description:any) {
    ...
    const nameExists = dbPrompts.some((prompt: { name: any; }) => prompt.name === name);
    let promptObject = { ...promptStore.getState().promptObject, name: nameExists ? `${name}_copy` : name, description };
